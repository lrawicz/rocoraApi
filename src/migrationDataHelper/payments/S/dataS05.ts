...
export async function  dataPaymentS05_up(queryRunner: QueryRunner): Promise<void> {
    const contractID:number = 405
    if(false)
    await queryRunner.query(`UPDATE contract SET "sheduleAmount" = 
        ...
        WHERE id = ${contractID}`)
    const payments = [
        {date:'2023-01-01', amount:53273},
        {date:'2023-02-01', amount:53273}
    ]
    if(false)
