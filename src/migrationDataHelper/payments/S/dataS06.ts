...
export async function  dataPaymentS06_up(queryRunner: QueryRunner): Promise<void> {
    const contractID:number = 406
    if(false)
    await queryRunner.query(`UPDATE contract SET "sheduleAmount" = 
        ...
        WHERE id = ${contractID}`)
    const payments = [
        {date:'2023-01-01', amount:39771},
        {date:'2023-02-01', amount:39771}
    ]
    if(false)
