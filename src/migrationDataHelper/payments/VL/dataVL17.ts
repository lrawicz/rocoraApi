...
export async function  dataPaymentVL17_up(queryRunner: QueryRunner): Promise<void> {
    const contractID:number = 117
    if(false)
    await queryRunner.query(`UPDATE contract SET "sheduleAmount" = 
        ...
        WHERE id = ${contractID}`)
    const payments = [
            {date:'2023-01-01',amount:28000},
            {date:'2023-02-01',amount:40000}
    ]
    if(false)
